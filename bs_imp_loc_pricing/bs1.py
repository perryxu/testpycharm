...
#    optnpv.set_index('CODE',inplace=True)
    ...
    dnt = ql.DoubleBarrierOption(ql.DoubleBarrier.KnockOut,1.0,9.8,0.0,
                                 ql.CashOrNothingPayoff(ql.Option.Call,0.0,1.0),
                                 ql.AmericanExercise(toql_date(rundate),ql.Date(18,10,2019),True))
    upnt = ql.BarrierOption(ql.Barrier.UpOut,2.93,0.0,
                                 ql.CashOrNothingPayoff(ql.Option.Put,2.93,1.0),
                                 ql.AmericanExercise(toql_date(rundate),ql.Date(18,10,2019),True))
    downnt = ql.BarrierOption(ql.Barrier.DownOut,2.7,0.0,
                                 ql.CashOrNothingPayoff(ql.Option.Call,2.7,1.0),
                                 ql.AmericanExercise(toql_date(rundate),ql.Date(18,10,2019),True))
